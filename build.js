 ...
 *
 *  3. Staging marketplace (--marketplace --staging) — same shape as the
 *     marketplace template but pointed at staging.facevault.id /
 *     api-staging.facevault.id, with hosted_base + api_base baked into
 *     web_view[0].meta. Used for FaceVault-internal staging exchanges.
 *
 * Usage:
 *   node build.js                            # generic template
 *   node build.js --marketplace              # marketplace template
 *   node build.js --marketplace --staging    # staging marketplace template
...
if (staging && !marketplace) {
	console.error('--staging requires --marketplace (staging shape is marketplace-only).');
	process.exit(1);
}
...
const plugin = {
	...
	description: staging
		? 'STAGING build. Identity verification powered by FaceVault — staging environment ' +
		  '(staging.facevault.id / api-staging.facevault.id). After install, set your slug in Configure.'
		: marketplace
	...
	web_view: [
		{
			...
			meta: Object.assign(
				...
				staging
					? {
						hosted_base: 'https://staging.facevault.id',
						api_base: 'https://api-staging.facevault.id',
					}
					: {}
			),
		},
...
const outFile = path.join(
	ROOT,
	staging
		? 'facevault-kyc.marketplace-staging.json'
		: marketplace
		? 'facevault-kyc.marketplace.json'
		: 'facevault-kyc.json'
);
